...
try {
    ch.runCommand("Executing Compose up...", args) { Process proc ->
        def (String out, String err) = DockerUtils.captureCommand(proc)
        if (err) {
            DockerUtils.error(err)
            System.exit(1)
        } else {
            apTool.setOutputProperty("OUTPUT_DATA", out)
